...
export async function updateUser(userId: string, username: string, updates: Partial<UserDB>) {
  const collection = await connectUserCollection();
  const result = await collection.updateOne({ userId, username }, { $set: updates });
  return result;
}
