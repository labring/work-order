...
const AppBaseInfo = ({ app }: { app: WorkOrderDB }) => {
  ...
  const { data: workorderInfo } = useQuery(
    ...
    {
      enabled: !!app?.orderId && session?.isAdmin
    }
  );
      ...
      {session?.isAdmin && (
        <Box mt="24px">
          ...
          >
            ...
            <Box onClick={() => copyData(workorderInfo?.user.username || '')}>
              username: {workorderInfo?.user?.username}
            </Box>
            <Box onClick={() => copyData(workorderInfo?.user?.teamId || '')}>
              teamId: {workorderInfo?.user?.teamId}
            </Box>
